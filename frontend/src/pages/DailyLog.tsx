...
export default function DailyLog() {
  ...
  const [logs, setLogs] = useState<DailyLog[]>([
    {
      ...
      title: '今天和布丁一起玩耍',
      content: '布丁今天特别活泼，我们一起玩了逗猫棒，它跳得很高，看起来很开心。',
      mood: 'happy',
      activities: ['玩耍', '运动'],
      photos: [],
  ...
  // 活动选项
  const activityOptions = ['玩耍', '运动', '休息', '喂食', '洗澡', '训练', '外出', '其他']
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{
                        ...
                        <span>·</span>
                        <span>{getMoodIcon(log.mood)} {getMoodName(log.mood)}</span>
                      </div>
                    </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div>
              <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#28a745' }}>
                {logs.filter(log => log.date === new Date().toISOString().split('T')[0]).length}
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                  {(['happy', 'normal', 'sad', 'excited', 'tired'] as DailyLog['mood'][]).map((mood) => (
                    <button
                      key={mood}
                      onClick={() => setNewLog(prev => ({ ...prev, mood }))}
                      style={{
                        background: newLog.mood === mood ? '#FFBF6B' : '#f8f9fa',
                        border: '1px solid #ddd',
                        borderRadius: '8px',
                        padding: '8px 12px',
                        fontSize: '14px',
                ...
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {activityOptions.map((activity) => (
                    <button
                      ...
                      style={{
                        background: newLog.activities.includes(activity) ? '#FFBF6B' : '#f8f9fa',
                        border: '1px solid #ddd',
                        borderRadius: '12px',
                        padding: '4px 8px',
                        fontSize: '12px',
