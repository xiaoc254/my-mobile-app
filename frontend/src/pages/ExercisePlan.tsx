...
export default function ExercisePlan() {
  ...
  const togglePlanCompletion = (planId: string) => {
    setExercisePlans(prev => prev.map(plan => 
      plan.id === planId 
        ? { 
            ...plan, 
  ...
  // 计算完成率
  const completionRate = exercisePlans.length > 0 
    ? Math.round((exercisePlans.filter(plan => plan.completed).length / exercisePlans.length) * 100)
    : 0
        ...
        <div style={{
          ...
          {exercisePlans.length === 0 ? (
            <div style={{
              ...
              <p>还没有运动计划</p>
              <p style={{ fontSize: '12px', marginTop: '5px' }}>点击上方按钮添加计划</p>
            </div>
          ) : (
              ...
              {exercisePlans.map((plan) => (
                <div 
                  ...
                  style={{
                    ...
                    border: plan.completed ? '1px solid #e9ecef' : '1px solid #f0f0f0',
                    borderRadius: '12px',
                    padding: '15px',
                    display: 'flex',
                    alignItems: 'center',
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div>
              <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#007bff' }}>
                {exercisePlans.reduce((total, plan) => total + (plan.completed ? plan.duration : 0), 0)}
              </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <div>
                ...
                <select 
                  value={newPlan.type}
                  onChange={(e) => setNewPlan(prev => ({ ...prev, type: e.target.value as ExercisePlan['type'] }))}
                  style={{
                    width: '100%',
                    padding: '12px',
                    border: '1px solid #ddd',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <input 
                  ...
                  onChange={(e) => setNewPlan(prev => ({ ...prev, reminder: e.target.checked }))}
