...
export default function PetAgeWeight() {
  ...
  const handleDateConfirm = (value: Date) => {
    const formattedDate = `${value.getFullYear()}/${String(value.getMonth() + 1).padStart(2, '0')}/${String(value.getDate()).padStart(2, '0')} ${String(value.getHours()).padStart(2, '0')}:${String(value.getMinutes()).padStart(2, '0')}`
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              width: '100%',
              height: '55px',
              background: (!startDate.trim() || !weight.trim() || isSubmitting) ? '#ccc' : '#FFD700',
              border: 'none',
              borderRadius: '25px',
              color: '#fff',
              fontSize: '18px',
              fontWeight: 'bold',
              cursor: (!startDate.trim() || !weight.trim() || isSubmitting) ? 'not-allowed' : 'pointer',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              boxShadow: (!startDate.trim() || !weight.trim() || isSubmitting) ? '0 2px 6px rgba(0, 0, 0, 0.1)' : '0 4px 12px rgba(255, 215, 0, 0.3)',
              transition: 'all 0.3s ease',
              opacity: (!startDate.trim() || !weight.trim() || isSubmitting) ? 0.6 : 1
            }}
            onMouseDown={(e) => {
              if (startDate.trim() && weight.trim()) {
